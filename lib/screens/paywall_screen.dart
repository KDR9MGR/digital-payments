...
class _PaywallScreenState extends State<PaywallScreen> {
  ...
  Future<void> _checkInitialSubscriptionStatus() async {
    try {
      ...
      if (cachedStatus && mounted) {
        AppLogger.log(
          'PaywallScreen: User has cached active subscription, navigating back immediately',
        );
        Navigator.of(context).pop();
        return;
      }

      // Then do a force refresh in background to ensure accuracy
      final hasSubscription = await _subscriptionService.isUserSubscribed(
        forceRefresh: true,
      );
      if (hasSubscription && mounted) {
        AppLogger.log(
          'PaywallScreen: User has active subscription after refresh, navigating back',
        );
        Navigator.of(context).pop();
      }
    } catch (e) {
      AppLogger.log(
        'PaywallScreen: Error checking initial subscription status: $e',
      );
      // Don't block the user if there's an error - let them see the paywall
    }
  }
  ...
  Future<void> _loadProductsOptimized() async {
    ...
    // CRITICAL FIX: Check if service is already initialized to avoid redundant initialization
