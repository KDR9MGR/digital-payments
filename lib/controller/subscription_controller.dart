...
class SubscriptionController extends GetxController {
  ...
  void onInit() {
    ...
    _subscriptionStatusSubscription = _subscriptionService
        ...
        .listen((hasSubscription) {
          AppLogger.log(
            'SubscriptionController: Received subscription status update: $hasSubscription',
          );
          _hasActiveSubscription.value = hasSubscription;
          _subscriptionStatus.value = hasSubscription ? 'active' : 'inactive';
          AppLogger.log(
            'SubscriptionController: Updated local subscription status to: ${_hasActiveSubscription.value}',
          );
  ...
  Future<void> _checkSubscriptionStatus() async {
    try {
      ...
      AppLogger.log(
        'SubscriptionController: Premium access granted to all authenticated users',
      );
  ...
  Future<void> processGooglePaySubscription() async {
    // PREMIUM ACCESS: All users now have premium access
    Get.snackbar(
      'Already Premium',
      'You already have premium access! No subscription needed.',
      backgroundColor: Colors.green,
      colorText: Colors.white,
    );
    return;

    // Original subscription processing code (commented out)
