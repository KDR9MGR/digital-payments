...
import 'package:xpay/services/internal_transfer_service.dart';
...
class _TransactionHistoryScreenState extends State<TransactionHistoryScreen>
  ...
  // Flag to determine data source (API vs Firestore)
  bool _useApiDataSource = true;
  ...
  Widget _buildTransactionList() {
    ...
    // Use API data source if enabled, otherwise fallback to Firestore
    if (_useApiDataSource) {
      return FutureBuilder<Map<String, dynamic>>(
        future: _getPaymentHistoryFromAPI(),
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError) {
            // Fallback to Firestore on API error
            return _buildFirestoreTransactionList(user.uid);
          }

          final result = snapshot.data;
          if (result == null || result['success'] != true) {
            // Fallback to Firestore if API call failed
            return _buildFirestoreTransactionList(user.uid);
          }

          final transactions = result['transactions'] as List<dynamic>? ?? [];
          final filteredTransactions = _filterApiTransactions(transactions);
          ...
          if (filteredTransactions.isEmpty) {
            return Center(
              child: Column(
                ...
                children: [
                  Icon(Ionicons.receipt_outline, color: Colors.white.withValues(alpha: 0.3), size: 64),
                  const SizedBox(height: 16),
                  Text(
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              return _buildApiTransactionCard(filteredTransactions[index]);
            },
          );
        },
      );
  ...
  Widget _buildFirestoreTransactionList(String userId) {
    return StreamBuilder<QuerySnapshot>(
      stream: _getTransactionsStream(userId),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return Center(
            child: CircularProgressIndicator(color: CustomColor.primaryColor),
  ...
  // Fetch payment history from the new API endpoint
  Future<Map<String, dynamic>> _getPaymentHistoryFromAPI() async {
    try {
      List<TransactionModel> transactions = await _transferService.getPaymentHistoryViaAPI();
  ...
  // Filter API transactions based on search and filter criteria
  List<dynamic> _filterApiTransactions(List<dynamic> transactions) {
  ...
  // Build transaction card for API data
  Widget _buildApiTransactionCard(dynamic transaction) {
    final data = transaction as Map<String, dynamic>;
    final amount = (data['amount'] ?? 0.0).toDouble();
    final currency = data['currency'] ?? 'USD';
    final recipientEmail = data['recipientEmail'] ?? 'Unknown';
    ...
    DateTime timestamp;
    try {
      final timestampStr = data['timestamp'] ?? data['createdAt'];
