...
class UserProvider with ChangeNotifier {
  ...
  Future<void> fetchUserDetails() async {
    try {
      ...
      if (result != null) {
        ...
        // Load card and bank data after user details are fetched
        await _loadUserPaymentData();
  ...
  void updateUserDirectly(UserModel? userModel) {
    if (userModel != null) {
      _user = userModel;
      // Load payment data when user is updated
      _loadUserPaymentData();
      notifyListeners();
    }
  }

  // Load user's card and bank account data
  Future<void> _loadUserPaymentData() async {
    try {
      ...
      AppLogger.log('Error loading user payment data: $e');
      // Controllers will fallback to local storage on error
    }
  }
