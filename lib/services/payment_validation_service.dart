...
class PaymentValidationService {
  ...
  static void startPaymentSession() {
    ...
    _startSessionTimer();
    AppLogger.log('Payment session started with ${_sessionTimeoutMinutes}min timeout');
  }
  ...
  static Future<Map<String, dynamic>> validateGooglePayPayment({
    ...
    try {
      ...
      final isDuplicate = await _checkDuplicateTransaction(transactionId, user.uid);
      if (isDuplicate) {
        return {
          'success': false,
  ...
  /// Check for duplicate transactions
  static Future<bool> _checkDuplicateTransaction(String transactionId, String userId) async {
    try {
  ...
  static Future<void> _storePaymentValidation(Map<String, dynamic> data) async {
    try {
      await _batchService.addWrite(
        collection: 'payment_validations',
        documentId: FirebaseFirestore.instance.collection('payment_validations').doc().id,
        data: data,
      );
  ...
  static Future<Map<String, dynamic>> _validateSubscriptionPlan(String planId, double amount) async {
    try {
