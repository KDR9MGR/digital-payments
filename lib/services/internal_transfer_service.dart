...
class InternalTransferService {
  ...
  /// Send money using the new payment processing endpoints
  Future<Map<String, dynamic>> sendMoneyViaPaymentAPI({
    ...
    try {
      ...
      if (amount < 0.50) {
        throw Exception('Minimum transfer amount is \$0.50');
      }
      ...
      if (!RegExp(r'^[^@]+@[^@]+\.[^@]+$').hasMatch(receiverEmail)) {
      ...
      AppLogger.log('Sending money via payment API: ${currentUser.email} -> $receiverEmail, Amount: $amount $currency');

      // Send money via new payment API
      ...
      if (response.isSuccess) {
        ...
        final transferId = response.data?['transfer_id'];
        AppLogger.log('Money sent successfully: Transaction ID: $transactionId, Transfer ID: $transferId');

        // Create local transaction record for tracking
        final timestamp = DateTime.now().millisecondsSinceEpoch;
        final localTransactionId = '${timestamp}_${currentUser.uid.substring(0, 8)}_payment_send';
        ...
        transactionData['receiver_email'] = receiverEmail;
        transactionData['backend_transaction_id'] = transactionId;
        transactionData['backend_transfer_id'] = transferId;
        transactionData['backend_response_data'] = response.data;
        ...
        return {
          ...
          'currency': currency,
          'receiverEmail': receiverEmail,
          'message': response.data?['message'] ?? 'Money sent successfully',
          'data': response.data,
        };
      } else {
        AppLogger.log('Money transfer failed: ${response.error}');
        return {
          'success': false,
      ...
      AppLogger.error('Send money error: $e', tag: 'InternalTransferService');
      return {
        'success': false,
        'error': e.toString(),
      };
  ...
  /// Get payment history using the new payment processing endpoints
  Future<List<TransactionModel>> getPaymentHistoryViaAPI() async {
    try {
      User? currentUser = _auth.currentUser;
      if (currentUser == null) {
        throw Exception('User not authenticated');
      ...
      AppLogger.info(
        'Getting payment history via API for user: ${currentUser.uid}',
        tag: 'InternalTransferService',
      );

      // Get payment history from new backend API
      final response = await _apiService.getPaymentHistory();

      if (response.isSuccess && response.data != null) {
        final List<dynamic> transactionsData = response.data!['transactions'] as List<dynamic>? ?? [];
        
        // Convert API response to TransactionModel objects
        final List<TransactionModel> transactions = transactionsData.map((transactionData) {
          return TransactionModel.fromMap({
            ...
            'receiverEmail': transactionData['receiver_email'] ?? '',
            'senderEmail': transactionData['sender_email'] ?? '',
            'description': transactionData['description'] ?? '',
            'status': transactionData['status'] ?? 'completed',
            'timestamp': transactionData['created_at'] != null 
                ? Timestamp.fromDate(DateTime.parse(transactionData['created_at']))
                : Timestamp.now(),
            'metadata': {
              'transfer_id': transactionData['transfer_id'] ?? '',
              'stripe_payment_intent_id': transactionData['stripe_payment_intent_id'] ?? '',
              'backend_data': transactionData,
            },
          });
        }).toList();

        AppLogger.log('Retrieved ${transactions.length} payment transactions');
        return transactions;
      } else {
        AppLogger.log('Failed to get payment history: ${response.error}');
        return [];
      }
    } catch (e) {
      AppLogger.error('Get payment history error: $e', tag: 'InternalTransferService');
      return [];
    }
  }
