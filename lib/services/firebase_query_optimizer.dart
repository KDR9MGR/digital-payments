...
class FirebaseQueryOptimizer {
  ...
  Future<Map<String, Map<String, dynamic>>> batchGetUsers(
    List<String> userIds,
  ) async {
    ...
    if (uncachedIds.isNotEmpty) {
      try {
        ...
        AppLogger.log(
          'Batch fetched ${uncachedIds.length} users from Firestore',
        );
      } catch (e) {
  ...
  Future<List<Map<String, dynamic>>> getTransactionHistory(
    ...
    final cacheKey =
        'transactions_${userId}_${limit}_${lastDocument?.id ?? 'first'}';
  ...
  Future<List<Map<String, dynamic>>> searchUsers(
    ...
    try {
      ...
      if (searchTerm.replaceAll(RegExp(r'[^0-9]'), '').length >= 10 &&
          searchFields.contains('phone')) {
        final phoneSnapshot =
            await _firestore
                .collection('users')
      ...
      AppLogger.log(
        'Search completed for: $searchTerm, found ${finalResults.length} results',
      );
  ...
  Future<Map<String, dynamic>?> getDailyAnalytics(DateTime date) async {
    ...
    try {
      final doc =
          await _firestore.collection('daily_analytics').doc(dateStr).get();
      if (doc.exists) {
        final data = doc.data()!;
  ...
  StreamSubscription<QuerySnapshot> listenToUserTransactions(
    ...
    AppLogger.log(
      'Setting up real-time listener for user transactions: $userId',
    );
    ...
    return _firestore
        ...
        .listen((snapshot) {
          ...
          AppLogger.log(
            'Real-time update: ${transactions.length} transactions for $userId',
          );
        });
  }
  ...
  void _cacheQuery(String key, List<Map<String, dynamic>> data) {
    _queryCache[key] = _CachedQuery(data: data, timestamp: DateTime.now());
  ...
  List<Map<String, dynamic>>? _getQueryFromCache(String key) {
    ...
    if (cached != null &&
        DateTime.now().difference(cached.timestamp) < _queryTtl) {
      return cached.data;
    }
  ...
  void _cleanExpiredQueries() {
    ...
    _queryCache.removeWhere(
      (key, cached) => now.difference(cached.timestamp) > _queryTtl,
    );
  }
  ...
  Future<List<Map<String, dynamic>>> getMoneyRequests(
    ...
    try {
      ...
      AppLogger.log(
        'Fetched and cached ${allRequests.length} requests for: $userEmail',
      );
  ...
  Future<bool> checkActiveSubscription(
    ...
    if (!forceRefresh) {
      ...
      if (cached != null) {
        AppLogger.log(
          'Retrieved active subscription status from cache: $userId',
        );
        return cached;
      }
    }
    ...
    try {
      ...
      AppLogger.log(
        'Fetched and cached active subscription status for: $userId',
      );
...
class _CachedQuery {
  ...
  _CachedQuery({required this.data, required this.timestamp});
}
