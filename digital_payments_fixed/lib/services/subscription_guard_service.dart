...
class SubscriptionGuardService {
  ...
  Future<bool> hasAccess() async {
    try {
      ...
      final hasActiveSubscription = await _subscriptionService.isUserSubscribed(forceRefresh: true);
  ...
  Widget guardWidget({
    ...
    return FutureBuilder<bool>(
      ...
      builder: (context, snapshot) {
        ...
        if (!hasAccess) {
          return fallbackWidget ?? _buildSubscriptionRequiredWidget(featureName);
        }
  ...
  void _showSubscriptionRequiredDialog(String featureName) {
    Get.dialog(
      AlertDialog(
        ...
        content: Text('$featureName requires a premium subscription. Upgrade now to unlock all features!'),
        actions: [
          TextButton(
            onPressed: () => Get.back(),
            child: const Text('Cancel'),
  ...
  void _showErrorDialog() {
    Get.dialog(
      AlertDialog(
        ...
        content: const Text('Unable to verify access permissions. Please try again.'),
        actions: [
          ElevatedButton(
            onPressed: () => Get.back(),
            child: const Text('OK'),
