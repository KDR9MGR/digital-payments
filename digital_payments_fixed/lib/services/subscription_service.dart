...
class SubscriptionService {
  ...
  Future<void> initialize() async {
    ...
    if (_isAvailable && _products.isNotEmpty) {
      AppLogger.log('Subscription service already initialized, skipping...');
      return;
    }
    ...
    for (final PurchaseDetails purchaseDetails in purchaseDetailsList) {
      if (purchaseDetails.status == PurchaseStatus.pending) {
        ...
        if (purchaseDetails.status == PurchaseStatus.error) {
          // Handle purchase error with better user messaging
          final errorMessage = purchaseDetails.error?.message ?? '';
          AppLogger.log('Purchase error: $errorMessage');

          // Filter out technical errors and provide user-friendly messages
          String userMessage;
          if (errorMessage.toLowerCase().contains('cancelled') ||
              errorMessage.toLowerCase().contains('user_cancelled')) {
            // Don't show error for user cancellation
            AppLogger.log(
              'Purchase cancelled by user, not showing error snackbar',
            );
            return;
          } else if (errorMessage.toLowerCase().contains('network') ||
              errorMessage.toLowerCase().contains('connection')) {
            userMessage =
                'Network error during payment. Please check your connection.';
