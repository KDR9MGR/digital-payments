...
class SubscriptionController extends GetxController {
  ...
  Future<void> _loadCustomerId() async {
    try {
      final user = FirebaseAuth.instance.currentUser;
      if (user == null) {
        // Log authentication error silently - user will be redirected to login
        AppLogger.log('User not authenticated for subscription features');
        return;
      }
      ...
      // Use proper error handling instead of generic error message
      await SubscriptionErrorHandler().handleSubscriptionError(
        errorType: 'service_unavailable',
        errorMessage: 'Payment system initialization failed',
        context: {'screen': 'subscription', 'action': 'initialize_payment'},
      );
    }
  }
  ...
  Future<bool> _subscribeWithMoov() async {
    if (_moovAccountId.value.isEmpty) {
      // Log account error silently and handle gracefully
      AppLogger.log('Moov account not found for user, will attempt to create');
      return false;
    }
    ...
    try {
      ...
      if (plan == null) {
        // Log plan error silently and handle gracefully
        AppLogger.log('Subscription plan not found: $singlePlanId');
        return false;
      }
  ...
  Future<bool> addPaymentMethod() async {
    if (_customerId.value.isEmpty) {
      // Log customer error silently and handle gracefully
      AppLogger.log('Customer not found, will attempt to create');
      return false;
    }
  ...
  Future<void> processApplePaySubscription() async {
    if (!_applePayAvailable.value) {
      // Show user-friendly message for Apple Pay unavailability
      Get.snackbar(
        'Payment Method Unavailable',
        'Apple Pay is not set up on this device. Please use another payment method.',
    ...
    try {
      ...
      if (paymentResult != null && paymentResult['success'] == true) {
        ...
        PaymentValidationService.endPaymentSession();
        // Show user-friendly message for Apple Pay failure
        Get.snackbar(
          'Payment Failed',
          'Apple Pay payment could not be completed. Please try again or use another payment method.',
          snackPosition: SnackPosition.TOP,
