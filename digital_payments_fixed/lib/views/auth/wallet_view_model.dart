      ...
      await ThreadingUtils.runFirebaseOperation(() async {
        ...
        if (userSnapshot.exists && userSnapshot.data() != null) {
          ...
          final transactionId = DateTime.now().millisecondsSinceEpoch.toString() + 
                                '_' + user.userId.substring(0, 8) + '_add';
  ...
  Future<void> fetchTransactionHistory({bool forceRefresh = false}) async {
    ...
    try {
      await ThreadingUtils.runFirebaseOperation(() async {
        ...
        if (firebaseUser == null) {
          if (kDebugMode) {
            AppLogger.warning('WalletViewModel.fetchTransactionHistory: No authenticated user found', tag: 'WalletViewModel');
          }
          return;
        }
        ...
        if (!forceRefresh) {
          final cachedTransactions = await _cacheService.getCachedTransactions(firebaseUser.uid);
          if (cachedTransactions != null && cachedTransactions.isNotEmpty) {
            _transactions = cachedTransactions
                ...
                .toList();
            AppLogger.log('Loaded ${_transactions.length} transactions from cache');
            return;
          }
        }
        ...
        await _cacheService.cacheTransactions(firebaseUser.uid, transactionData);
        AppLogger.log('Fetched and cached ${_transactions.length} transactions from optimized query');
  ...
  Future<void> sendMoneyToUserWithMoov(
    ...
    try {
      await ThreadingUtils.runFirebaseOperation(() async {
        ...
        final senderTransactionId = transferId ?? 
            (DateTime.now().millisecondsSinceEpoch.toString() + '_' + sender.userId.substring(0, 8) + '_send');
        final recipientTransactionId = DateTime.now().millisecondsSinceEpoch.toString() + 
            '_' + recipient.userId.substring(0, 8) + '_receive';
