...
class _PaywallScreenState extends State<PaywallScreen> {
  ...
  Future<void> _loadProductsOptimized() async {
    ...
    // Quick cache check first - show cached products immediately
    ...
    try {
      ...
      await _subscriptionService.initialize().timeout(
        ...
        onTimeout: () {
          throw Exception('Service initialization timeout');
        },
      );
      ...
      // Handle specific error types without showing unexpected prompts
      String errorMessage;
  ...
  Future<void> _purchaseSubscription() async {
    AppLogger.log(
      'PaywallScreen: Subscribe Now button clicked - starting purchase process',
    );
    ...
    try {
      ...
      final hasActiveSubscription =
          await _subscriptionService.isUserSubscribed();
      ...
      if (!_subscriptionService.isAvailable) {
        ...
        _showErrorDialog(
          'In-app purchases are not available. Please check your connection and try again.',
        );
        return;
      }

      AppLogger.log(
        'PaywallScreen: Calling subscription service to purchase: $_selectedProductId',
      );

      // Add timeout to prevent hanging purchases
      final success = await _subscriptionService
          .purchaseSubscription(_selectedProductId!)
          .timeout(
            const Duration(seconds: 30),
            onTimeout: () {
              AppLogger.log('PaywallScreen: Purchase request timeout');
              throw Exception('Purchase request timeout');
            },
          );
      ...
      if (success) {
        ...
        _showSuccessDialog(
          'Purchase initiated! Please complete the payment in the App Store.',
        );
      } else {
        AppLogger.log('PaywallScreen: Purchase initiation failed');
        _showErrorDialog(
          'Failed to initiate purchase. Please try again or contact support if the issue persists.',
        );
      }
      ...
      // Handle specific error types to reduce unexpected prompts
      String errorMessage;
      ...
      if (errorString.contains('timeout')) {
        ...
        errorMessage =
            'Network error during purchase. Please check your internet connection.';
      } else if (errorString.contains('billing') ||
          errorString.contains('payment')) {
        errorMessage =
            'Payment processing error. Please check your payment method and try again.';
  ...
  void _listenForPurchaseCompletion() {
    // Listen to subscription status changes with timeout and proper cleanup
    late StreamSubscription subscription;
    Timer? timeoutTimer;

    subscription = _subscriptionService.subscriptionStatusStream.listen((
      hasSubscription,
    ) {
    ...
    timeoutTimer = Timer(const Duration(seconds: 60), () {
      subscription.cancel();
      if (mounted) {
        AppLogger.log('PaywallScreen: Purchase completion timeout');
        _showErrorDialog(
          'Purchase verification is taking longer than expected. Please check your subscription status in Settings.',
        );
      }
    });
  }
                      ...
                      SizedBox(
                        ...
                        child: Builder(
                          builder: (context) {
                            ...
                            AppLogger.log(
                              'PaywallScreen: Rendering Subscribe button - Products: ${_products.length}, Selected: $_selectedProductId, Enabled: $isEnabled, Loading: $_isLoading, Text: $buttonText',
                            );
                            ...
                            return ElevatedButton(
                              onPressed:
                                  isEnabled
                                      ? () {
                                        AppLogger.log(
                                          'PaywallScreen: Subscribe Now button pressed',
                                        );
