...
func Login(c *gin.Context) {
	...
	// TODO: Implement actual user authentication with your user store
	// For now, this is a placeholder
	userID := uuid.New().String()

	token, err := GenerateJWT(userID, loginRequest.Email, "")
	if err != nil {
...
func Register(c *gin.Context) {
	...
	token, err := GenerateJWT(userID, registerRequest.Email, "")
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to generate token"})
		return
	}
