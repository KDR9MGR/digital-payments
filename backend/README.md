...
### Payment Processing
- `POST /api/v1/payments/send` - Send money to another user
- `POST /api/v1/payments/receive` - Receive money from another user
...
### Receive Money
```bash
curl -X POST http://localhost:8080/api/v1/payments/receive \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer <your-jwt-token>" \
  -d '{
...
### Get Payment History
```bash
curl -X GET http://localhost:8080/api/v1/payments/history \
  -H "Authorization: Bearer <your-jwt-token>"
```
